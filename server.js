...
import serviceAccountKey from './blog-bda62-firebase-adminsdk-i3zta-ecbe134b1d.json' assert {type: 'json'}
...
server.get("/get-upload-url", (req, res) => {
    generateUploads().then(url => res.status(200).json({ uploadURL: url }))
        .catch(err => {
...
server.post("/signin", (req, res) => {
    ...
    User.findOne({ "personal_info.email": email })
        .then(user => {
            ...
            //  return   res.status(201).json({ "status": "got user document" })
        })
...
server.post("/google-auth", async (req, res) => {
    ...
    getAuth()
        ...
        .then(async (decodedUser) => {
            ...
            picture = picture.replace("s96-c", "sc384-c")
            ...
            if (user) { //login
                if (user.google_auth) {
                    return res.status(500).json({ "error": "This email was signed up without google. please log in with password to access the account" })
                }
            } else { //sign up
                let username = await generateUsername(email)
                ...
                await user.save().then((u) => user = u).catch((err) => {
                    return res.status(500).json({ "error": err.message })
                })
            }
            return res.status(200).json(formatDatatoSend(user))
        }).catch((err) => {
            return res.status(500).json({ "error": "Failed to authenticate you with google. Try with some order google account" })
        })
})
    ...
    User.findOne({ _id: req.user })
        .then((user) => {
            ...
            bcrypt.compare(currentPassword, user.personal_info.password, (err, result) => {
                ...
                if (!result) {
                    return res.status(403).json({ error: "Incorrect currrent Password" })
                }
                ...
                bcrypt.hash(newPassword, 10, (err, hash_password) => {
                    User.findOneAndUpdate({ _id: req.user }, { "personal_info.password": hash_password })
                        ...
                        .catch(err => {
                            return res.status(500).json({ error: "Some error occured while saving new password, pleasetry again later" })
                        })
                })
            })
        })
...
server.post("/update-profile", verifyJWT, (req, res) => {
    ...
    if (username.length < 3) {
        return res.status(403).json({ error: "Username should ba at least 3 letters long" })
    }
        ...
        for (let i = 0; i < socialLinksArr.length; i++) {
            if (social_links[socialLinksArr[i]].length) {
                ...
                if (!hostname.includes(`${socialLinksArr[i].com}`) && socialLinksArr[i] != 'website') {
                    return res.status(403).json({ error: `${socialLinksArr[i].com} link is invalid. You must enter a full link` })
                }
            }
        }
...
server.post("/add-comment", verifyJWT, (req, res) => {
    ...
    if (!comment.length) {
        return res.status(403).json({ error: "Write something leave a comment" })
    }
...
server.post("/user-written-blog-count", verifyJWT, (req, res) => {
    let user_id = req.user
...
.then(blog =>{
    ...
    let incrementVal = blog.draft ? 0 : -1;
    User.findOneAndUpdate({_id:user_id}, {$pull :{blog:blog_id}, $inc: {"account_info.total_posts": incrementVal}})
...
.catch(err =>{
    return res.status(200).json({error:err.messagee})
})
// }
// else{
//     return res.status(200).json({error:"you don't have permissions to delete the blog"})
// }
})
